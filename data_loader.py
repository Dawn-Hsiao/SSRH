...
class ImageLoader_for_test(data.Dataset):
    def __init__(self, data_dir, image_size, w_path, transform=None):
        ...
        self.data_dir = data_dir
        self.transform = transform
        self.img_paths = natsort.natsorted(os.listdir(data_dir), alg=natsort.ns.PATH)
        self.image_size = image_size
    ...
    def __getitem__(self, index):
        ...
        Data_img = Data_img.transpose((2, 0, 1))
        Data_img = np.float32(Data_img / 255 * 2 - 1)
        return Data_img, m, num, name


def get_loader(image_dir, image_size=128,
               batch_size=32, dataset='train_mask', mode='train_mask', num_workers=1,
               w_path='results/WatermarkMatrix/w.mat'):
    """Build and return a data loader."""
    transform = []

    if dataset in ['test_accuracy', 'test_embedding']:
